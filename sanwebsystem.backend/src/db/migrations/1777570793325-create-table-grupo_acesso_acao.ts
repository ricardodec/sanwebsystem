...
export class CreateTableGrupoAcessoAcao1777570793325 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE IF NOT EXISTS grupo_acesso_acao (
                ...
                CONSTRAINT FK_grupo_acesso_acao_componente
                    ...
                    REFERENCES acao_componente (componente_ID , acao_ID)
                        ON DELETE NO ACTION
                        ON UPDATE NO ACTION,
                CONSTRAINT FK_grupo_acesso_acao_grupo_acesso_ID
                    FOREIGN KEY (grupo_acesso_ID)
                    REFERENCES grupo_acesso (ID)
                        ON DELETE NO ACTION
                        ON UPDATE NO ACTION)
            ENGINE = InnoDB;`,
        );
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP TABLE grupo_acesso_acao;`);
    }
}
