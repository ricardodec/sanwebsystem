...
# instalar o certbot para gerar o certificado HTTPS para quem tem domínio
...
# Usar o pm2 para manter o app sempre aberto e iniciando junto com o sistema.

```sh
npm install -g pm2
cd /home/ricar/sanwebsystem/sanwebsystem.frontend
pm2 start npm --name sanwebsystem -- start
pm2 save
...
# Ver
pm2 list # listar
pm2 restart sanwebsystem # reinicia
pm2 stop sanwebsystem # para
pm2 start sanwebsystem # inicia
pm2 log sanwebsystem # veja o log do next (importante para debug)
...
pm2 delete sanwebsystem # reinicia
pm2 start npm --name sanwebsystem -- start -i max # modo cluster, 1 instância por core do processador
pm2 save
```
...
server {
  ...
  # --- LOGS ---
  access_log /var/log/nginx/sanwebsystem.access.log;
  error_log /var/log/nginx/sanwebsystem.error.log;
  ...
  # location /admin {
  #   allow 123.123.123.124; # Se quiser restringir um IP específico
...
# Atualizar automaticamente no seu servidor pra você e já reinicia os servidores:

```sh
nano ~/refresh-sanwebsystem.sh
```
...
```sh
sudo chmod +x ~/refresh-sanwebsystem.sh
```

# Do seu computador, executar o seguinte comando mudando os dados para o seu servidor (usuário, domínio ou ip, nome do arquivo):

```sh
ssh ricar@sanwebsystem.com.br './refresh-sanwebsystem.sh'
```
